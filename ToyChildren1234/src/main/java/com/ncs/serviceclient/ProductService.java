...
public class ProductService {
	...
	public ResponseData<GetListProductOutput> getListProducts(ProductListInput input) {
		...
		try {
			...
			if(input.getPriceStart() != null) priceStart = input.getPriceStart();
			if(input.getPriceEnd() != null) priceEnd = input.getPriceEnd();

			response.setData(productDao.getListProduct(input.getPage(), input.getSize(), input.getSearch(), category, priceStart, priceEnd));
		} catch (Exception e) {
			LOGGER.error("Api get list product has exception : {}", e.getMessage());
			response.setCode(Constants.UNKNOWN_ERROR_CODE);
			response.setMessage(Constants.UNKNOWN_ERROR_MSG);
